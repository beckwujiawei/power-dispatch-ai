from __future__ import annotations

from datetime import datetime
from pathlib import Path
import json

from fastapi import FastAPI, HTTPException, Query
from fastapi.responses import FileResponse
from fastapi.staticfiles import StaticFiles
from pydantic import BaseModel, Field

from app.agent.communication_agent import CommunicationAgent
from app.schemas import ChatRequest, DispatchResponse
from app.services.dispatch_service import get_mock_alerts
from app.services.model_service import get_model_status
from app.services.report_service import build_handover_report
from app.services.ticket_service import (
    VALID_STATUSES,
    create_ticket,
    list_tickets,
    update_ticket_status,
)

BASE_DIR = Path(__file__).resolve().parent.parent
STATIC_DIR = BASE_DIR / "static"
AUDIT_FILE = BASE_DIR / "audit_logs.json"

app = FastAPI(
    title="电力通信调度智能助手",
    description="第一版演示系统，不连接真实生产系统。",
    version="0.7.0",
)
app.mount("/static", StaticFiles(directory=STATIC_DIR), name="static")
agent = CommunicationAgent()


class AuditRequest(BaseModel):
    session_id: str = "demo-session"
    action: str
    content: str
    operator: str = "演示调度员"


class TicketCreateRequest(BaseModel):
    title: str = Field(min_length=1)
    priority: str = "一般"
    station: str = ""
    related_device: str = ""
    description: str = ""
    operator: str = "演示调度员"
    session_id: str = "demo-session"


class TicketStatusRequest(BaseModel):
    status: str
    operator: str = "演示调度员"
    session_id: str = "demo-session"


class OperationModeRequest(BaseModel):
    mode_name: str
    operator: str = "演示调度员"
    session_id: str = "demo-session"


OPERATION_MODES = [
    {
        "id": "normal",
        "name": "正常运行模式",
        "status": "active",
        "description": "标准通信网络运行方式，保持稳定供电和业务通信链路正常。",
        "coverage": "全网",
        "last_updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    },
    {
        "id": "degraded",
        "name": "受限运行模式",
        "status": "standby",
        "description": "部分链路受限，需加强告警跟踪和设备巡检。",
        "coverage": "重点站点",
        "last_updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    },
    {
        "id": "maintenance",
        "name": "检修保障模式",
        "status": "standby",
        "description": "计划检修期，优先保障关键通信链路和现场协同。",
        "coverage": "检修区域",
        "last_updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    },
]


def write_audit_log(audit_request: AuditRequest) -> dict:
    if AUDIT_FILE.exists():
        try:
            logs = json.loads(AUDIT_FILE.read_text(encoding="utf-8"))
            if not isinstance(logs, list):
                logs = []
        except (json.JSONDecodeError, OSError):
            logs = []
    else:
        logs = []

    log_item = {
        "id": len(logs) + 1,
        "session_id": audit_request.session_id,
        "operator": audit_request.operator,
        "action": audit_request.action,
        "content": audit_request.content,
        "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    }
    logs.append(log_item)
    AUDIT_FILE.write_text(json.dumps(logs, ensure_ascii=False, indent=2), encoding="utf-8")
    return log_item


@app.get("/")
def index():
    return FileResponse(STATIC_DIR / "index.html")


@app.get("/api/health")
def health():
    return {"status": "ok", "service": "power-dispatch-ai", "mode": "demo", "model": get_model_status()}


@app.get("/api/config/model")
def model_config():
    return get_model_status()


@app.get("/api/alerts")
def alerts(
    station: str | None = Query(default=None),
    level: str | None = Query(default=None),
    status: str | None = Query(default=None),
):
    items = get_mock_alerts()
    if station:
        items = [item for item in items if item.station == station]
    if level:
        items = [item for item in items if item.level == level]
    if status:
        items = [item for item in items if item.status == status]
    return {"items": items, "total": len(items), "filters": {"station": station, "level": level, "status": status}}


@app.post("/api/dispatch/chat", response_model=DispatchResponse)
def dispatch_chat(request: ChatRequest):
    result = agent.handle(
        request.message,
        session_id=request.session_id or "demo-session",
        operator="演示调度员",
    )
    return result.to_dispatch_response()


@app.post("/api/audit/confirm")
def confirm_audit(request: AuditRequest):
    return {"success": True, "message": "确认记录已保存。", "item": write_audit_log(request)}


@app.get("/api/audit/logs")
def get_audit_logs():
    if not AUDIT_FILE.exists():
        return {"items": [], "total": 0}
    try:
        logs = json.loads(AUDIT_FILE.read_text(encoding="utf-8"))
        if not isinstance(logs, list):
            logs = []
    except (json.JSONDecodeError, OSError):
        logs = []
    return {"items": logs, "total": len(logs)}


@app.get("/api/reports/handover")
def get_handover_report():
    report = build_handover_report()
    return report


@app.get("/api/operation/modes")
def get_operation_modes():
    return {"items": OPERATION_MODES, "current": next((m for m in OPERATION_MODES if m["status"] == "active"), OPERATION_MODES[0])}


@app.post("/api/operation/modes/activate")
def activate_operation_mode(request: OperationModeRequest):
    for mode in OPERATION_MODES:
        mode["status"] = "standby"
    target = next((m for m in OPERATION_MODES if m["name"] == request.mode_name or m["id"] == request.mode_name), None)
    if target is None:
        raise HTTPException(status_code=404, detail="运行方式不存在")
    target["status"] = "active"
    target["last_updated"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    write_audit_log(AuditRequest(
        session_id=request.session_id,
        operator=request.operator,
        action="切换运行方式",
        content=f"切换至 {target['name']}。",
    ))
    return {"success": True, "item": target}


@app.post("/api/tickets")
def create_ticket_endpoint(request: TicketCreateRequest):
    ticket = create_ticket(request.model_dump(exclude={"operator", "session_id"}), request.operator)
    write_audit_log(AuditRequest(
        session_id=request.session_id,
        operator=request.operator,
        action="确认并创建工单",
        content=f"工单 {ticket['id']}：{ticket['title']}",
    ))
    return {"success": True, "message": "工单已创建。", "item": ticket}


@app.get("/api/tickets")
def get_tickets(status: str | None = Query(default=None)):
    if status and status not in VALID_STATUSES:
        raise HTTPException(status_code=400, detail=f"不支持的工单状态：{status}")
    items = list_tickets(status)
    return {"items": items, "total": len(items), "statuses": sorted(VALID_STATUSES)}


@app.patch("/api/tickets/{ticket_id}/status")
def change_ticket_status(ticket_id: str, request: TicketStatusRequest):
    try:
        ticket = update_ticket_status(ticket_id, request.status)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    if ticket is None:
        raise HTTPException(status_code=404, detail="工单不存在")
    write_audit_log(AuditRequest(
        session_id=request.session_id,
        operator=request.operator,
        action="更新工单状态",
        content=f"工单 {ticket_id} 状态更新为：{request.status}",
    ))
    return {"success": True, "message": "工单状态已更新。", "item": ticket}


@app.get("/api/tickets/{ticket_id}")
def get_ticket_detail(ticket_id: str):
    items = list_tickets()
    for ticket in items:
        if ticket.get("id") == ticket_id:
            return {"item": ticket}
    raise HTTPException(status_code=404, detail="工单不存在")


@app.get("/api/summary")
def summary():
    alerts = get_mock_alerts()
    tickets = list_tickets()
    return {
        "alerts_total": len(alerts),
        "serious_alerts": sum(1 for item in alerts if item.level == "严重"),
        "tickets_total": len(tickets),
        "tickets_open": sum(1 for item in tickets if item.get("status") != "已关闭"),
        "system_status": "正常",
    }


if __name__ == "__main__":
    import uvicorn

    uvicorn.run(app, host="0.0.0.0", port=8000)

